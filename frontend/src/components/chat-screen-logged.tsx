// components/chat-screen-logged.tsx - VERSÃO COMPLETA CORRIGIDA
...
interface ChatScreenLoggedProps {
  onBack: () => void;
...
export function ChatScreenLogged({
  onBack,
  onRecipeGenerated,
  initialIngredients,
  initialImages,
}: ChatScreenLoggedProps) {
  ...
  useEffect(() => {
    console.log("🔍 useEffect - Verificando imagens (logado):", {
    ...
    const sendInitialIngredients = async () => {
      console.log("🔍 Verificando envio automático (logado):", {
      ...
      if (
        ...
        try {
          console.log("🚀 Enviando imagens automaticamente para IA (logado)");
  ...
  const handleReanalyzeImages = async () => {
    console.log("🔄 Forçando reanálise das imagens (logado)");
    hasSentInitialIngredients.current = false;
    previousImagesCount.current = initialImages.length;
      ...
      <header className="h-[60px] flex items-center gap-4 px-4 border-b border-border">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-[#6C757D]">
            Usuário Logado - Suas receitas serão salvas automaticamente
          </p>
        </div>
        ...
        {conversation.length > 0 && initialImages.length > 0 && !isLoading && (
          <button
            ...
            disabled={isLoading}
            className="touch-target px-3 py-2 bg-green-100 text-green-700 rounded-lg text-sm hover:bg-green-200 transition-colors disabled:opacity-50 flex items-center gap-2"
        ...
        {conversation.length === 0 && !isLoading && (
          <div className="text-center py-8">
            ...
            <div className="space-y-2">
              <button
                onClick={async () => {
                  console.log("🔄 Fallback manual - usuário logado");
                  hasSentInitialIngredients.current = false;
                  try {
                    await sendMessage(
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200/50">
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200 shadow-sm">
                      ...
                      <Button
                        onClick={() => {
                          console.log(
                            "🖱️ CLICOU - Receita (usuário logado):",
                            message.recipe
                          );
                          onRecipeGenerated(
                            message.recipe,
